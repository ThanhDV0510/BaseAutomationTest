...
public class Organization_Product extends WebActions {

	public Organization_Product(WebDriver driver) {
		super(driver);
     }
	
	By lblProductList = By.id("label-product-list");
	By btnAddProduct = By.id("createProduct");
	By txtSearch = By.xpath("(//div[@id='product-list-grid']//input)[1]");
	By iconEdit = By.xpath("(//button[@id='action-edit'])[1]");
	By iconDelete = By.xpath("(//button[@id='action-delete'])[1]");
	By iconPlus = By.xpath("(//button[@id='action-create'])[1]");
	By itemFirst = By.xpath("(//div[@id='product-list-grid']//td/span[2])[1]");
	...
	public boolean isLblProductListDisplayed()
	{
		boolean flag = false;
		try {
			if (driver.findElements(lblProductList).size() > 0) {
				flag = true;
			}
		} catch (Exception e) {
			return flag;
